# The GUI code is stole from another github repo, dont remember which one, did this at 5 am soo....
...
class Player(Tk.Frame):
    ...
    def __init__(self, parent, title=None, hostServer = None, client=None):
    ...
    def OnOpen(self):
        ...
        if os.path.isfile(fullname):
            ...
            # * I removed these two lines so I could full screen the video....
    ...
    def SetTime(self, _time):
        ...
        """
        if self.player is None or abs(self.player.get_time() - _time) < 800:
            return

        self.player.set_time(_time)

        """if self.player == None or abs(self.player.get_time() - _time) < 250:
            return
    ...
    def OnTimer(self):
        ...
        if self.hostServer is not None:
            _packet = packet.serializeTime(t)
            self.hostServer.broadcastToRoom("", _packet)
